...
window.addEventListener('scroll', () => {
  ...
  const totalFotos = 95;
  const alturaMaxima = backgroundAnimation.clientHeight;
  // const contagemFotos = parseInt(totalFotos / alturaMaxima);
  const contagemFotos =
    parseInt(totalFotos / alturaMaxima) <= 0
      ? 1
      : parseInt(totalFotos / alturaMaxima);
  ...
  if (elementoTop <= 0 && elementoBottom >= 0) {
    ...
    requestAnimationFrame(() => updateImage(frameIndex * -1 + contagemFotos));
